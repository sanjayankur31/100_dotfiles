...
function fetchdeps() {
    REQUIREPACKAGES=$(grep -rhiI "requirepackage" *.tex *.cls *.sty 2>/dev/null | sed -e "s|^.*\\\RequirePackage.*{|tex(|" -e "s|}.*$|\.sty)|" | sed -e "s|,|.sty) tex(|" | uniq -u | tr '\n' ' ')
    REQUIREPACKAGES=${REQUIREPACKAGES}
    USEPACKAGES=$(grep -rhiI "usepackage" *.tex *.cls *.sty 2>/dev/null | sed -e "s|^.*\\\usepackage.*{|tex(|" -e "s|}.*$|\.sty)|" | sed -e "s|,|.sty) tex(|" | uniq -u | tr '\n' ' ')
