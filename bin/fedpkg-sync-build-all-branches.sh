...
impact_check () {
    ...
    mapfile -t DEPS < <(fedrq whatrequires-src -b "${branch}" -X "${PACKAGE_NAME}")
    for dep in "${DEPS[@]}"
    do
        echo "*** ${dep} ***"
...
usage () {
    echo "Usage: "
    echo "$0 [-utIip] <branches to sync with rawhide>"
    echo
    echo "Options:"
    ...
    echo "-p <package name>: specify package for impact check (when not in a package git folder)"
    echo
    echo "Positional parameters:"
    echo "<branches> branches to run impact check for, or sync from rawhide"
}
...
while getopts "Iiu:t:p:h" OPTION
do
    case $OPTION in
        u)
            UPDATE_NOTE="$OPTARG"
...
if [ ${#branches[@]} -eq 0 ]
then
    echo ">> No branches supplied. Exiting"
    usage
    exit 1
fi
...
then
    ...
    then
        ...
        echo "Could not find a spec file in this folder."
        echo "Use -p <package-name> to specify the package for impact checks."
        exit 1
    fi
else
    echo ">> Package name provided: ${PACKAGE_NAME}"
    echo ">> Running in impact-check-only mode. To merge and build, run from a package git folder."
    IMPACT_CHECK="Yes"
fi
